...
### 2026.01.30 // Dynamic log cards
We did a massive cleanup to align with the "Zero-Clutter" philosophy. No more generic cards, no more deep nesting.

- **Structure Flattened**: all logs moved to the root level (`/[slug]`). The `/logs` directory is gone. URLs are shorter and cleaner.
- **Flow State**: The main feed is now fluid. Pinned Logs are now moved to the Sidebar, so the main feed is purely for content flow.
